...
public class ESClient {
    ...
    /**
     * 功能描述：根据ID查询一条数据记录。
     * @param idList 要查询数据的ID列表。
     * @return 返回查询出来的记录对象的json字符串。
     */
    public List<Map<String, Object>> get(String index, String type, Collection<String> idList) {
        List<Map<String, Object>> list = new ArrayList<>();
        MultiGetRequestBuilder multiGetRequestBuilder = client.prepareMultiGet();
        for (String _id : idList) {
            multiGetRequestBuilder.add(index, type, _id);
        }
        Iterator<MultiGetItemResponse> iterator = multiGetRequestBuilder.get().iterator();
        while (iterator.hasNext()) {
            MultiGetItemResponse next = iterator.next();
            list.add(next.getResponse().getSource());
        }
        return list;
    }
    ...
    public Map<String, Object> statSearch(ESQueryConstructor constructor, String statBy, String index, String... type) {
        ...
        if (null != constructor) {
            ...
            constructor = new ESQueryConstructor();
    ...
    /**
     * 功能描述：根据时间戳获取该时间对应的type
     * @author qiaobin
     * @date 2017/4/25  17:19
     * @param
     */
    public String[] getSameTypes(String index, long timestamp) {
        Object[] types = getTypes(index);
        SimpleDateFormat sdf = new SimpleDateFormat(SIMPLE_FORMATTER);
        Timestamp ts = new Timestamp(timestamp);
        String typePrefix = sdf.format(ts);
        ...
        String[] sameTypes = null;
        if (samePrefixType.size() > 0) {
            sameTypes = new String[samePrefixType.size()];
            for (int i = 0; i < samePrefixType.size(); i++) {
                sameTypes[i] = samePrefixType.get(i);
            }
        }
        return sameTypes;
    }
